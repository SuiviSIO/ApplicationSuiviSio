...
class Model extends Modele {
  ...
  // recherche  s'il existe un mel identique à $mel
  ...
  //si $num=integer recherche sauf pour $num (=si $num poss�de ce mel, c'est normal)
    ...
    if ($res[0]["nb"]>0) { //ce mél est déjà enregistré dans prof
      return true;
    } else {
...
*/
  ...
  public function getBilanSitu($processus,$groupe){
    ...
    //récupère les élèves du groupe :num,nom,prenom
    $tb=$this->getElevesGroupe($groupe);

    //récupère toutes les situations de l'atelier :id,nomen,lib,ln
    $tbs=$this->getSitusAtelier($atelier);

    //pour chaque élève, recherche de situations validées
    for($i=0;$i<count($tb);$i++){
      $valeurs=array();
  ...
  public function suppPersonne($vers,$num){
    if ($vers==1){// prof
      ...
      $req='delete from '.$this->pref.'professeur
            where num='.$num;
      $this->mysql->execSQL($req);
    } else {//$vers=2 suppression de tout ce qui concerne l'étudiant
  ...
  public function setProf($nom,$prenom,$mel,$niveau,$smtp,$crypte){
    if ($this->existeMel($mel,null)) { //ce mél est déjà enregistré
      return false;
    } else {
      if ($crypte && !$smtp)
      	$mdp=$this->mdpdefaut;
  ...
  public function getSitusClEl($i){
    ...
    for ($j=0;$j<count($tb);$j++){
      ...
      if ($tb1[0]["nb"]>0) {
        ...
        //dévalidation forcée pour éliminer la situation fausse à problème :
      ...
      //retourne les ref des situations comportant de compétences citées >1 fois
...
*/
  ...
  public function getSitus($id,$gr,$t){
    //le prof voit tous les étudiants de son groupe

    //$t=1 : situations récentes (dateModif > dateCommentaireLe+R�cent) non comment�es
    //$t=2 : situations récentes (dateModif > dateCommentaireLe+R�cent)
    //$t=3 : non comment�es
    //$t=4 : toutes
...
*/
  ...
  public function getTableauSyntheseNew($numeleve){
