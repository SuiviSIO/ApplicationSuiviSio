...
if (@opendir('dirrw/init/')) {
  ...
  if (!isset($_REQUEST["action"])){
    ...
    if (in_array($act,$cl,TRUE) && ($util->estAuthent() || $act="authent")){
      ...
      elseif ($act=='synthese'){ /***** ajout� par le LLB le 26-06-12 *****/
	$type = $_GET["type"]; //$type recevra comme valeur f ou s1 ou s2
	$actm=ucfirst($act);
        $ctrl = new $actm($util, $type, $_REQUEST); //ajout du param�tre $type dans le constructeur de la classe Synthese (voir ctrl/Synthese.class.php)
      }
      else {
      	$actm=ucfirst($act);
