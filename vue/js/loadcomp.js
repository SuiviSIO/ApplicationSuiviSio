...
//impossible de faire d�claration Array(val1, val2,...)
//(1 �lem = dimension ; +1�lem = valeurs)
...
function enleve() {//supprime activit�(s) donc compets
	...
	for (i=frmchoia.length -1 ; i>=0 ; i--)
		if (frmchoia.options[i].selected){
			...
			var trouve=false;
			for (j=0 ; j<choix.length ; j++)//d�cale les n� activ dans choix
				if (choix[j]==value)
					trouve=true;
				else
					if (trouve)
...
function activsel() {//s�lectionne ou non les comp en fonction des activ s�lect ou non
	var frmchoia = document.frmSitu.activchoisies;
	var frmchoic = document.frmSitu.compchoisies;
	var value;
	var prem = true;
...
function majchoix(){
	ch='';
	if (choix.length == 1)//join ne marche pas pour 1 �lem ?
		ch=choix[0];
	else
		ch=choix.join(',');
